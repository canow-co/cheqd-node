...
var _ = Describe("cheqd cli - negative transfert token", func() {
	It("cannot transfer token with missing cli arguments", func() {
		...
		_, err := cli.TransferToken("", testdata.BASE_ACCOUNT_2_ADDRESS, amount, cli.CliGasParams)
		Expect(err).ToNot(BeNil())

		//    b. missing receiver account address
		_, err = cli.TransferToken(testdata.BASE_ACCOUNT_1_ADDRESS, "", amount, cli.CliGasParams)
		Expect(err).ToNot(BeNil())

		//    c. we cannot use receiver account name instead of receiver account address
		_, err = cli.TransferToken(testdata.BASE_ACCOUNT_1_ADDRESS, testdata.BASE_ACCOUNT_2, amount, cli.CliGasParams)
		Expect(err).ToNot(BeNil())
		...
		//    e. missing amount
		_, err = cli.TransferToken(testdata.BASE_ACCOUNT_1_ADDRESS, testdata.BASE_ACCOUNT_2_ADDRESS, "", cli.CliGasParams)
		Expect(err).ToNot(BeNil())

		AddReportEntry("Integration", fmt.Sprintf("%sNegative: %s", cli.PURPLE, "cannot transfer token with invalid amount"))
		//    a. invalid amount
		_, err = cli.TransferToken(testdata.BASE_ACCOUNT_1_ADDRESS, testdata.BASE_ACCOUNT_2_ADDRESS, fmt.Sprintf("-1%s", testdata.DEMON), cli.CliGasParams)
		Expect(err).ToNot(BeNil())

		_, err = cli.TransferToken(testdata.BASE_ACCOUNT_1_ADDRESS, testdata.BASE_ACCOUNT_2_ADDRESS, fmt.Sprintf("0%s", testdata.DEMON), cli.CliGasParams)
		Expect(err).ToNot(BeNil())
	})
})
